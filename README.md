...
| 主动对话 | 桌宠**主动发起话题**，可配置空闲触发间隔与话题偏好，让互动更自然 |
...
| 记忆体 | 用户手动维护的全局固定记忆条目 + **自动沉淀候选**（会话结束后自动抽取，需确认后生效）+ **记忆档案面板**（按角色/分类浏览、搜索、批量管理） |
...
她不再只是被动应答——她会根据设定主动发起话题：

- **触发条件**：可配置空闲时长阈值，超过后她自动发起对话
- **话题偏好**：设置中可开启/关闭主动对话，调整触发间隔
- **无缝集成**：主动消息与正常对话完全同步，支持流式输出与语音合成
- **尊重用户**：对话进行中不会重复触发，用户可随时关闭此功能
