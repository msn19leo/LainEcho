 ...
 * 主动搭话调度器。
 *
 * - 调度循环：每 30s 一轮；兴趣值每轮 +5~10（上限 100），>50 后按 p=(interest-50)/50 掷骰触发；
...
async function tryDeliver(kind: ProactiveIntentKind, settings: AppSettings, st: PersistState): Promise<boolean> {
  ...
  const narration = await buildNarration(kind, settings)
...
export function startProactiveScheduler(): void {
  ...
  console.log('[proactive] 调度器已启动（tick=%ds）', TICK_MS / 1000)
}
